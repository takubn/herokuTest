...
<body onload="moveBottom(),changeColorByParam()">
  ...
  <?php require_once "php/get_init.php"?>
    ...
    <?php for ($i = 1; $i <= $paging_number; $i++): ?>
      <li><a href="?page=<?=$i?>" id="pager<?=$i?>"><?=$i?></a></li>
    <?php endfor;?>
...
?>
    ...
    <?php for ($i = 0; $i < $max; $i++): ?>
      <div class="display-box">
        ...
        <!-- 編集ボタン -->
        <button class="buttonEdit" onclick="changeFormEdit(withoutStrName<?=$i;?>,contents<?=$i;?>),changeEditMode(name<?=$i;?>,contents<?=$i;?>,primalyKey<?=$i;?>),postEdit(name<?=$i;?>,contents<?=$i;?>,primalyKey<?=$i;?>)">edit</button>
        <!-- 削除ボタン　-->
